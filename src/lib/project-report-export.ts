...
export async function getApprovedReportContext(supabase: SupabaseClient, projectTeamId: string) {
  ...
  if (projectTeamError || !projectTeam) {
    throw new ReportExportError("対象データが見つかりません。", 404);
  }

  if (!isApprovedLike(projectTeam.status)) {
    throw new ReportExportError("承認済みの提出のみExcel出力できます。", 409);
  }
...
export async function generateProjectReport(
  ...
  context: Awaited<ReturnType<typeof getApprovedReportContext>>
) {
  const { projectTeamId } = context;
  ...
  if (companyError || summaryError || detailError || reportError || fileError) {
    throw new ReportExportError("报告资料读取失败，请重试。未生成或上传不完整的报告。");
  }
...
export async function recordProjectReportExport(supabase: SupabaseClient, projectTeamId: string) {
  const { error: exportRecordError } = await supabase
    .from("project_teams")
    .update({ exported_at: new Date().toISOString() })
    .eq("id", projectTeamId);
  if (exportRecordError) {
    throw new ReportExportError("Excel出力日時の保存に失敗しました。再度お試しください。");
  }
}
